...
1. **ETP Mensuelle (Thornthwaite classique) :**
   - Calcul basé sur la température moyenne mensuelle (`T_mens`) et la photopériode théorique mensuelle.
...
3. **Gestion Automatique de la Latitude :**
   ...
   - **Ajustement astronomique automatique :** Les formules de déclinaison solaire et le calcul de la durée du jour s'adaptent mathématiquement en coulisses sans que l'utilisateur n'ait à se soucier d'appliquer des signes négatifs pour des latitudes situées dans l'hemisphère Sud.
...
- `T_mens` : Température moyenne mensuelle (°C). (Correspond à la colonne `T_moy_mens`). Si `< 0`, on retient `0`.
...
2. **Calcul de l'ETP mensuelle globale (`ETP_mens`) :** Calculée avec la méthode classique détaillée ci-dessus.
